...
| **M1 — Model + spec store** | ⬜ Pending | — | — | Extend `specNodeFrontmatterSchema` in place (ADR-008): typed `contracts[]`, optional given/when/then. Brings the first YAML dependency. **Also renames the project directory `.v6r/` → `.vousoir/`** — `V6R_ROOT_DIRNAME`'s value only; no symbol or filename changes. Manifest is JSON. |
...
all are kept with their answers in [`ADR.md`](./ADR.md#open-questions). **One question remains, and it
was generated by those answers:**
