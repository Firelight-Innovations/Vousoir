...
- Open `/extensions/css-language-features/` as the workspace in Vousoir
...
- Run the [`Launch Extension`](https://github.com/microsoft/vscode/blob/master/extensions/css-language-features/.vscode/launch.json) debug target in the Debug View. This will:
	- Launch a new Vousoir instance with the `css-language-features` extension loaded
...
- Debug the language server process by using `Attach to Node Process` command in the  Vousoir window opened on `css-language-features`.
...
This extension wraps the css language service into a Language Server for Vousoir.
