...
- Open `/extensions/json-language-features/` as the workspace in Vousoir
...
- Run the [`Launch Extension`](https://github.com/microsoft/vscode/blob/master/extensions/json-language-features/.vscode/launch.json) debug target in the Debug View. This will:
	- Launch a new Vousoir instance with the `json-language-features` extension loaded
...
- Debug the language server process by using `Attach to Node Process` command in the  Vousoir window opened on `json-language-features`.
