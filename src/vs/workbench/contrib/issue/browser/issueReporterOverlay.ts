...
export class IssueReporterOverlay {
	...
	private createStep0Attachments(): void {
		...
		subtitle.textContent = localize('screenshotsSubtitle', "You can add up to {0} screenshots or videos. Navigate Vousoir and choose when to capture.", MAX_ATTACHMENTS);
	...
	private getAllSourceOptions(): { label: string; value: IssueSource }[] {
		return [
			{ label: product.nameLong || localize('vscode', "Vousoir"), value: IssueSource.VSCode },
			{ label: localize('agentsWindow', "Agents Window"), value: IssueSource.AgentsWindow },
			{ label: localize('extensionSource', "A Vousoir extension"), value: IssueSource.Extension },
	...
	private getIssueSourceLabel(): string {
		switch (this.selectedIssueSource) {
			case IssueSource.VSCode:
				return product.nameLong || localize('vscode', "Vousoir");
			case IssueSource.AgentsWindow:
				return localize('agentsWindow', "Agents Window");
			case IssueSource.Extension:
				return this.selectedExtension?.displayName || this.selectedExtension?.name || localize('extensionSource', "A Vousoir extension");
			case IssueSource.Marketplace:
