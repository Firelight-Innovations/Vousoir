	...
	private async checkForTrustedPublishers(extensions: { extension: IGalleryExtension; manifest: IExtensionManifest; checkForPackAndDependencies: boolean }[]): Promise<void> {
		...
		const learnMoreButton: IPromptButton<void> = {
			...
			run: () => {
				...
				this.instantiationService.invokeFunction(accessor => accessor.get(ICommandService).executeCommand('vscode.open', URI.parse('https://github.com/Firelight-Innovations/Vousoir')));
				throw new CancellationError();
			}
		};
		...
		const unverifiedLink = 'https://github.com/Firelight-Innovations/Vousoir';
