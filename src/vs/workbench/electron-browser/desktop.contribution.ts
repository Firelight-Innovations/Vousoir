	...
	registry.registerConfiguration({
		...
		'properties': {
			'application.shellEnvironmentResolutionTimeout': {
				...
				'markdownDescription': localize('application.shellEnvironmentResolutionTimeout', "Controls the timeout in seconds before giving up resolving the shell environment when the application is not already launched from a terminal. See our documentation for more information.")
			}
		}
	});
	...
	const schema: IJSONSchema = {
		...
		properties: {
			...
			'disable-chromium-sandbox': {
				type: 'boolean',
				description: localize('argv.disableChromiumSandbox', "Disables the Chromium sandbox. This is useful when running Vousoir as elevated on Linux and running under Applocker on Windows.")
			},
			'use-inmemory-secretstorage': {
				type: 'boolean',
				description: localize('argv.useInMemorySecretStorage', "Ensures that an in-memory store will be used for secret storage instead of using the OS's credential store. This is often used when running Vousoir extension tests or when you're experiencing difficulties with the credential store.")
			},
